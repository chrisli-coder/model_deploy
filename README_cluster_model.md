...
- **`NixlConnector` only:** the script adds **`kv_connector_extra_config`** with fixed **`backends: ["UCX"]`**, **`side_channel_host`** set to that node’s **`RDMA_IPS[i]`** entry, and **`peer_ips`** as the RDMA IPs of all nodes in the **opposite** PD role (prefill ↔ decode). Other connectors get only `kv_role` + `kv_connector` in JSON unless extended later.
